...
void app_user_msg_handler(ke_msg_id_t const msgid, void const *p_param)
{
    ...
    switch (msgid)
    {
       ...
       case APP_SWITCH_MODE:
           ...
           HP_LOG_DEBUG("APP_SWITCH_MODE\r\n");
           if(fw_mode == 0)
            {
                HP_LOG_INFO("Master mode\r\n");
                //Sstart scan
                hp_ble_start_scan();
            }
            else {
                HP_LOG_INFO("Slave mode\r\n");
                // Start Advertising
                hp_ble_adv_start();
            }
           break;
