...
void user_send_ble_data(const uint8_t *data, uint16_t length)
{
    ...
    if(hp_ble_master_connection_num())
    {
        ...
        req->cursor = hp_ble_get_conidx_by_role(ROLE_MASTER);
        req->length = length;
    ...
    {
        HP_LOG_INFO("Connect error with peripheral device\r\n");
    }
}
...
{
    HP_LOG_DEBUG("%s\r\n", __func__);
    HP_LOG_INFO("RDTSC profile was enabled = %x\r\n", param->status);
...
{
    HP_LOG_DEBUG("%s\r\n", __func__);
    HP_LOG_DEBUG("Data from connected peripheral\r\n");
