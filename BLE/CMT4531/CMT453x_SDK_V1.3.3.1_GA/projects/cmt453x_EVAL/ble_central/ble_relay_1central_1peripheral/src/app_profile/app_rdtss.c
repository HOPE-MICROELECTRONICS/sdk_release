                                          ...
                                          ke_task_id_t const src_id)
{
    ...
    uint16_t length = ind_value->length;
		uint16_t conidx_m = hp_ble_get_conidx_by_role(ROLE_MASTER);
    ...
    switch (handle)
    {
        ...
        case RDTSS_IDX_WRITE_VAL:
            //ble data receive
            HP_LOG_INFO("Data from connected central\r\n");
        ...
        case RDTSS_IDX_CNTL_POINT_VAL:
            {
                ...
                switch(ind_value->value[0])
                {
                    case CNTL_POINT_OP_CONN:
                    {
                        ...
                        {
                            HP_LOG_INFO("ADDR type error: %x\r\n", ind_value->value[1]);
                        }
                        else
                        {
													if(conidx_m != GAP_INVALID_CONIDX)
													{
														HP_LOG_INFO("Master connetion has been established\r\n");
														rdtss_notify_ctrl_point_cmd_cmp(CNTL_POINT_OP_CONN, STATUS_ERR_MASTER_HAS_CONNECTED, NULL, 0);
													}
													else
													{
														     HP_LOG_INFO("Start connecting peripheral, index: %x\r\n", i);
                                HP_LOG_INFO("Target ADDR: ");
                                for(uint8_t k=0; k<6; k++)
                                {
                                    HP_LOG_INFO(" %x ", ind_value->value[k+2]);
                                }
                                HP_LOG_INFO("\r\n");
                                
                                hp_ble_stop_scan();
                                HP_LOG_INFO("Stop SCAN\r\n");
                                
                                hp_ble_start_init((uint8_t *)&(ind_value->value[2]), ind_value->value[1]);
													}
                        }
                    ...
                    case CNTL_POINT_OP_DISCONN:
                    {
                        ...
                        if(conidx_m != GAP_INVALID_CONIDX)
                        {
													 hp_ble_set_active_connection(conidx_m);
                           hp_ble_disconnect();
                        }
                        else
                        {
...
{
    if(hp_ble_slave_connection_num())
    {
        ...
        req->conidx = hp_ble_get_conidx_by_role(ROLE_SLAVE);
        req->notification = true;
        req->handle = RDTSS_IDX_NTF_VAL;
        req->length = length;
        memcpy(&req->value[0], data, length);

        HP_LOG_DEBUG("%x", req->value[0]);
        
        ke_msg_send(req);
    }
    else
    {
        HP_LOG_INFO("Not connected with central device\r\n");
    }
}
