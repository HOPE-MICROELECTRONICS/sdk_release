...
{
    HP_LOG_DEBUG("usart_rx_fifo_out = %x\r\n", usart_rx_fifo_out);
    HP_LOG_DEBUG("usart_rx_fifo_in = %x\r\n", usart_rx_fifo_in);
    ...
    ble_sending = true;    
    #if (BLE_RDTS_CLIENT)
    HP_LOG_INFO("ble_send_len = %x\r\n", ble_send_len);

    if (usart_rx_fifo_buf[usart_rx_fifo_out] < 3)
    {
        hp_ble_set_active_connection(usart_rx_fifo_buf[usart_rx_fifo_out]);
    }
...
void usart_rx_check_in_irq(void)
{
    ...
    HP_LOG_DEBUG("RX_POS = %x\r\n", rx_pos);
    HP_LOG_DEBUG("RX_OLD_POS = %x\r\n", rx_old_pos);
