...
#define AES_LOG HP_LOG_DEBUG  // uart ouput all AES test data will block BLE pairing
/**
...
void app_aes_ccm_cb(bool mic_error, uint32_t src_info)
{
    ...
    if(cipher == 1)
    {
        ...
        if(mic_error)
        {
            HP_LOG_INFO("Tag(mic) authentication fail!\r\n");
        }
        else{
            HP_LOG_INFO("Tag(mic) authentication pass!\r\n");
        }        
        if(memcmp(test_input,test_plain,16*(DATA_BLOCK-1)) == 0)
        {
            HP_LOG_INFO("CCM data decrypt pass!\r\n");
        }
        else{
            HP_LOG_INFO("CCM data decrypt fail!\r\n");
        }
...
void app_aes_ccm_demo(void)
{
    ...
    if(cipher)
    {
        ...
        hp_ase_swap_array(key_lsb,key,AES_BLOCK_SIZE);
        hp_aes_ccm(key_lsb, nonce, test_input[0], test_cipher[0], 16*(DATA_BLOCK-1), CCM_MIC_LEN , true, add_auth_data, 2, app_aes_ccm_cb, 0);
        
        //do the decrypt 2ms later
        hp_timer_create(5,app_aes_ccm_demo);
    }
    else{
        hp_ase_swap_array(key_lsb,key,AES_BLOCK_SIZE); 
        hp_aes_ccm(key_lsb, nonce, test_cipher[0], test_plain[0], 16*(DATA_BLOCK-1), CCM_MIC_LEN , false, add_auth_data, 2, app_aes_ccm_cb, 0);
        cipher = 1;
        //repeat cbc encrypt test 1000ms later
//        hp_timer_create(1000,app_aes_ccm_demo);
    }
}
...
void app_aes_cbc_cb(uint8_t status, const uint8_t* aes_res, uint32_t src_info)
{
    if(src_info == DECRYPT_SRC_MARK)
    {
        ...
        {
            HP_LOG_INFO("CBC encrypt and decrypt pass!\r\n");
        }
        else{
            HP_LOG_INFO("CBC encrypt and decrypt fail!\r\n");
        }
    }
    else{
        AES_LOG("cbc_cipher: \r\n");
...
void app_aes_cbc_demo(void)
{
    if(cipher)
    {
        ...
        hp_aes_cbc(key, iv, test_input[0],test_cipher[0],true, 16*DATA_BLOCK, app_aes_cbc_cb);
        
        //do the decrypt 2ms later
        hp_timer_create(5,app_aes_cbc_demo);
    }
    else{
        hp_aes_cbc(key, iv, test_cipher[0],test_plain[0],false, 16*DATA_BLOCK, app_aes_cbc_cb);
        cipher = 1;
        //repeat cbc encrypt test 1000ms later
        hp_timer_create(1000,app_aes_cbc_demo);
    }
}
...
void app_aes_result_cb(uint8_t status, const uint8_t* aes_res, uint32_t src_info)
{
    if(src_info == DECRYPT_SRC_MARK ) //decrypt
    {
        ...
        {
            HP_LOG_INFO("AES encrypt and decrypt pass!\r\n");
        }
        else{
            HP_LOG_INFO("AES encrypt and decrypt fail!\r\n");
        }
    }
    else{
...
void ase_test_demo(void)
{
    if(cipher)
    {
        ...
        hp_aes_ciphering(key,p_in,app_aes_result_cb);
        cipher = 0;
        
         //do the decrypt 2ms later
        hp_timer_create(2,ase_test_demo);
    }
    else{
        cipher = 1;
        
        hp_aes_deciphering(key,test_cipher[0], app_aes_result_cb);
        //repeat
//        hp_timer_create(500,ase_test_demo);
    }
}
...
void app_ble_msg_handler(struct ble_msg_t const *p_ble_msg)
{
    switch (p_ble_msg->msg_id)
    {
        case APP_BLE_OS_READY:
            ...
            hp_timer_create(10,ase_test_demo);
            hp_timer_create(1000,app_aes_cbc_demo);
            hp_timer_create(2000,app_aes_ccm_demo);
            break;
...
void app_ble_prf_init(void)
{
    ...
    hp_ble_add_prf_func_register(app_rdtss_add_rdts);
