...
#include "hp_sec.h"                // Application Security Module API
...
static uint8_t key_enter;
static uint8_t key_enter_offset = 0;
static const uint8_t key_array[KEY_ARRAY_SIZE] = {
  0x04, /* a */
  0x05,   /* b */
  0x06,   /* c */
  0x07,   /* d */
...
void app_hid_add_hids(void)
{
    ...
    // There are total 3 HIDS instance in the database
    db_cfg->hids_nb = 3;

    // The device is a mouse
    db_cfg->cfg[0].svc_features = HOGPD_CFG_MOUSE;//HOGPD_CFG_MOUSE_BIT;

    // Add 3 report characteristic instances to the database
...
void app_hid_enable_prf(uint8_t conidx)
{
    ...
    if (hp_sec_get_bond_status())
    {
        ...
        app_hid_env.nb_report = APP_HID_NB_SEND_REPORT;
		ntf_cfg = 0x1C2;	// Enable the notification automatically.
        HP_LOG_DEBUG("HID ready for bonded device\r\n");
...
void app_hid_send_mouse_report(struct ps2_mouse_msg report)
{
    ...
    HP_LOG_DEBUG("HID report, state:%d, x:%d, y:%d \r\n",app_hid_env.state, report.x, report.y);
...
void app_hid_send_keyboard_report(struct keyboard_msg report)
{
    ...
    HP_LOG_DEBUG("HID KB report, state:%d \r\n",app_hid_env.state);
    switch (app_hid_env.state)
    {
        case (APP_HID_READY):
        {
            ...
            {
                // Buffer used to create the Report
                uint8_t report_buff[APP_HID_KEYBOARD_REPORT_LEN];

                // Clean the report buffer
                memset(&report_buff[0], 0, APP_HID_KEYBOARD_REPORT_LEN);

								key_enter = key_array[key_enter_offset++];
								key_enter_offset = key_enter_offset%KEY_ARRAY_SIZE;							
							
                // Allocate the HOGPD_REPORT_UPD_REQ message for sending the characteristic
                struct hogpd_report_upd_req * req = KE_MSG_ALLOC_DYN(HOGPD_REPORT_UPD_REQ,
                                                                  prf_get_task_from_id(TASK_ID_HOGPD),
                                                                  TASK_APP,
                                                                  hogpd_report_upd_req,
                ...
                // Set the button states
                report_buff[0] = 0x00;				// Uppercase 0x02
								report_buff[1] = 0x00;
								report_buff[2] = key_enter;		//0x04;		// 'a'
								
                req->conidx  = app_hid_env.conidx;
                //now fill report
                req->report.hid_idx  = app_hid_env.conidx;
                req->report.type     = HOGPD_REPORT; 	//HOGPD_BOOT_MOUSE_INPUT_REPORT;//
                req->report.idx      = 2; 						//2 for keyboard reports and report map
                req->report.length   = APP_HID_KEYBOARD_REPORT_LEN;
                memcpy(&req->report.value[0], &report_buff[0], APP_HID_KEYBOARD_REPORT_LEN);

                ke_msg_send(req);
								
								// Allocate the HOGPD_REPORT_UPD_REQ message for sending the end
                struct hogpd_report_upd_req * reqx = KE_MSG_ALLOC_DYN(HOGPD_REPORT_UPD_REQ,
                                                                  prf_get_task_from_id(TASK_ID_HOGPD),
                                                                  TASK_APP,
                                                                  hogpd_report_upd_req,
                                                                  APP_HID_KEYBOARD_REPORT_LEN);
                ...
                reqx->conidx  = app_hid_env.conidx;
                //now fill report
                reqx->report.hid_idx  = app_hid_env.conidx;
                reqx->report.type     = HOGPD_REPORT; 	//HOGPD_BOOT_MOUSE_INPUT_REPORT;//
                reqx->report.idx      = 2; 						//2 for keyboard reports and report map
                reqx->report.length   = APP_HID_KEYBOARD_REPORT_LEN;
                memcpy(&reqx->report.value[0], &report_buff[0], APP_HID_KEYBOARD_REPORT_LEN);

                ke_msg_send(reqx);								

								HP_LOG_DEBUG("[%s][%d] nb_report %d, timeout %d \r\n", __FUNCTION__, __LINE__, app_hid_env.nb_report, app_hid_env.timeout);
                                     ...
                                     ke_task_id_t const src_id)
{
    ...
    if (app_hid_env.conidx == param->conidx)
    {
        ...
        else
        {
            ...
            HP_LOG_DEBUG("HID enable\r\n");
        }
        HP_LOG_DEBUG("ntf_cfg:0x%x\r\n",param->ntf_cfg[param->conidx]);    
        #if (NVDS_SUPPORT)
...
int app_hid_mouse_timeout_timer_handler(ke_msg_id_t const msgid,void const *param)
{
    ...
    if (app_hid_env.state == APP_HID_READY)
    {
         ...
         * Requested connection interval: 15ms
         * Latency: 20
         * Supervision Timeout: 2s
         */
        conn_param.intv_min = 12;
