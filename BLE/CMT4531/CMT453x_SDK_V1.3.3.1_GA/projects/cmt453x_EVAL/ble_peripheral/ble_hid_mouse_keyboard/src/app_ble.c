...
void app_ble_sec_msg_handler(struct sec_msg_t const * p_msg)
{
    switch (p_msg->msg_id)
    {
        case HP_SEC_PINCODE_DISPLAY:
            
            break;
        case HP_SEC_PAIR_SUCCEED:
            ke_timer_set(APP_PARAMS_UPDATE_EVT, TASK_APP, FIRST_CONN_PARAMS_UPDATE_DELAY);
            break;
        case HP_SEC_PAIR_FAILED:
...
void app_ble_gap_params_init(void)
{
    ...
    dev_info.appearance     = 0x03C0;							//ADV_APPEARANCE_HUMAN_INTERFACE_DEVICE_HID;
...
void app_ble_sec_init(void)
{
    ...
    sec_init.hp_sec_msg_handler     = app_ble_sec_msg_handler;
...
void app_ble_prf_init(void)
{
    ...
    hp_ble_add_prf_func_register(app_hid_add_hids);
