...
#include "l2cm_int.h"
#if (BLE_APP_PRESENT)
#include "hp_ble_task.h"             // Application Manager Task API
#include "hp_ble.h"                  // Application Manager Definition
#if (BLE_APP_SEC)
#include "hp_sec.h"              // Security Module Definition
#endif //(BLE_APP_SEC)
#if (HP_TIMER_ENABLE)
#include "hp_timer.h"
    ...
    switch(gattc_cmp_evt->operation)
    {
        case (GATTC_MTU_EXCH):
        {
            ...
            if(app_env.manual_mtu_update)
            {
                ...
                hp_dfu_ble_handler_mtu_update(gattc_cmp_evt->status);
            }
            #endif                
        ...
        {
            HP_LOG_DEBUG("GATTC_WRITE_NO_RESPONSE CMP\r\n");
                                ...
                                ke_task_id_t const src_id)
{
    ...
    {
        HP_LOG_ERROR("%s, op = %x, status = %x\r\n", __func__, p_param->operation, p_param->status);
    }
    else{
        HP_LOG_DEBUG("%s, op = %x, status = %x\r\n", __func__, p_param->operation, p_param->status);
    }
    ...
    switch(p_param->operation)
    {
        ...
        case (GAPM_SET_DEV_CONFIG):
        {
            ...
            #if (BLE_APP_SEC)  
            if (hp_sec_get_bond_status()==true)
            {
                hp_bond_last_bonded_ral_info(&dev_info);
        ...
        #if (BLE_APP_PRF)
        {
            ...
            #if(BLE_APP_SEC)
            if (hp_sec_get_bond_status()==false)
            {
            }
            #endif
            app_env.rand_cnt = 0;
        ...
        case (GAPM_RESOLV_ADDR):
        {
            ...
            if(p_param->status != CO_ERROR_NO_ERROR)
            {
                ...
                hp_bond_resolv_addr_start(&app_env.peer_addr,app_env.next_svc--);
            }
            #endif
        } break;
        ...
        case (GAPM_CREATE_INIT_ACTIVITY):
        {
            if( (app_env.init_actv_idx != 0xFF) && (p_param->status == CO_ERROR_NO_ERROR) )
            {
                ...
                {
                    hp_ble_start_init(app_env.target_addr.addr, app_env.target_addr_type);
                }
            }
            else
            {
...
static int gapc_cmp_evt_handler(ke_msg_id_t const msgid,
                                ...
                                ke_task_id_t const src_id)
{
    HP_LOG_DEBUG("%s, operation =%x status = %x\r\n", __func__,p_param->operation, p_param->status);
    if(app_env.ble_msg_handler)
    {
        struct ble_msg_t ble_msg = {APP_BLE_NULL_MSG,NULL,NULL};
    ...
    switch(p_param->operation)
    {
        case (GAPC_UPDATE_PARAMS):
        {
            ...
            if(app_env.manual_conn_param_update)
            {
                ...
                hp_dfu_ble_handler_conn_param_update(p_param->status);
            }
            #endif
        }
                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    HP_LOG_INFO("BLE Disconnected,reson:%x,conidx:%d \r\n",p_param->reason,KE_IDX_GET(src_id));    
...
static int gapm_addr_solved_ind_handler(ke_msg_id_t const msgid, 
                                          ...
                                          ke_task_id_t const src_id)
{
    HP_LOG_DEBUG("%s,src:%d\r\n",__func__,src_id);
    if(src_id == TASK_APP) //ke msg
    {
        //start solved
        hp_bond_search_addr_irk(&(app_env.peer_addr),app_env.peer_addr_type);
    }
    else
    {
        memcpy(app_env.peer_irk.key,p_param->irk.key ,GAP_KEY_LEN);
        #if 0 //debug
        HP_LOG_DEBUG("This mac solved:%02X %02X %02X %02X %02X %02X \r\n",
                                              p_param->addr.addr[0],
                                              p_param->addr.addr[1],
                                              p_param->addr.addr[2],
                                              p_param->addr.addr[3],
                                              p_param->addr.addr[4],
                                              p_param->addr.addr[5]);
        HP_LOG_DEBUG("This irk:");
        for(uint8_t i = 0; i<GAP_KEY_LEN;i++)
        {
            HP_LOG_DEBUG(" %02X",p_param->irk.key[i]);
        }
        HP_LOG_DEBUG("\r\n");
        #endif
    }
                                        ...
                                        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    hp_ble_update_param(&gap_env.dev_conn_param);
                                 ...
                                 ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    else
    {
        ...
        RCC->OSCFCCR |= 1;
        hp_sleep_lock_release(); //release when lsi calib finsh
    }   
...
#if (HP_TIMER_ENABLE)
    hp_timer_api_process_handler(msgid, param, dest_id, src_id,&process_msg_handling_result);
                                 ...
                                 ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    for(int i = 0; i < 6; i++)
    {
        HP_LOG_DEBUG("%02x ", param->trans_addr.addr.addr[i]);
    }
    switch (scan_env.filter_type)
    {
        ...
        case SCAN_FILTER_BY_NAME:
            if(hp_ble_scan_data_find(GAP_AD_TYPE_COMPLETE_NAME,scan_env.filter_data,(uint8_t*)param->data,param->length))
            {
                target_found = true;
            }
            break;
        case SCAN_FILTER_BY_UUID128:
            if(hp_ble_scan_data_find(GAP_AD_TYPE_SERVICE_128_BIT_DATA,scan_env.filter_data,(uint8_t*)param->data,param->length))
            {
                target_found = true;
            }
            break;
        case SCAN_FILTER_BY_UUID16:
            if(hp_ble_scan_data_find(GAP_AD_TYPE_SERVICE_16_BIT_DATA,scan_env.filter_data,(uint8_t*)param->data,param->length))
            {
                target_found = true;
            }
            break;
        case SCAN_FILTER_BY_APPEARANCE:
            if(hp_ble_scan_data_find(GAP_AD_TYPE_APPEARANCE,scan_env.filter_data,(uint8_t*)param->data,param->length))
            {
                target_found = true;
            }
            break;
    ...
    if(!app_env.target_found)
    {
        ...
        if(target_found)
        {
            HP_LOG_DEBUG("Found target device\r\n");
            if(scan_env.ble_scan_state_handler != NULL)
            {
                scan_env.ble_scan_state_handler(SCAN_STATE_FOUND);
            }            
            hp_ble_stop_scan();
            hp_ble_start_init((uint8_t*)param->trans_addr.addr.addr, param->trans_addr.addr_type);
...
#include  "app_rf.h"
static int app_rf_tx_end_msg_handler(ke_msg_id_t const msgid, void *p_param,
                                        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    HP_LOG_DEBUG("%s\r\n",__func__);
...
static int app_rf_rx_end_msg_handler(ke_msg_id_t const msgid, void *p_param,
                                        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    HP_LOG_DEBUG("%s\r\n",__func__);
    return (KE_MSG_CONSUMED);
...
struct  app_rf_rx_msg
{
    uint8_t length;
    uint8_t data[];
};
static int app_rf_rx_data_msg_handler(ke_msg_id_t const msgid, struct app_rf_rx_msg *p_param,
                                        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    HP_LOG_INFO("rx_len = %x\r\n",  p_param->length);
    for(int i = 0; i< p_param->length; i++)
    {
        HP_LOG_INFO("%02x ",  p_param->data[i]);
    }
    HP_LOG_INFO("\r\n");
    return (KE_MSG_CONSUMED);
}
...
static int app_rf_rx_wind_handler(ke_msg_id_t const msgid, void *p_param,
                                        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    //stop_rx
    app_rx_stop();
...
static int app_rf_intv_handler(ke_msg_id_t const msgid, void *p_param,
                                        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
		static uint16_t sentCnt = 0;
		HP_LOG_DEBUG("%s, RF_CTRL_MODE is %s \r\n", __func__, (RF_CTRL_MODE == RF_TX_ONLY)?"RF_TX_ONLY":"RF_RX_ONLY"); 
    
#if(RF_CTRL_MODE == RF_TX_ONLY)
		uint8_t tx_len  = 20;
		uint8_t tx_data[20];

		tx_data[0] = sentCnt & 0xFF;
		tx_data[1] = (sentCnt>>8) & 0xFF;
		for(int i = 2; i<tx_len; i++)
		{
				tx_data[i] = i-2;
		}
		app_tx_start(tx_len, tx_data);
		sentCnt++;
		
		ke_timer_set(APP_RF_INTV_TIMER, TASK_APP, RF_TX_INTV);
#else
		extern void app_rx_start(void);
		app_rx_start(); 

		// Only start the timer for stopping rx if RF_RX_WINDOW < RF_RX_INTV, otherwise, receiving forever.
		if(RF_RX_WINDOW < RF_RX_INTV){
			ke_timer_set(APP_RF_RX_WIND_TIMER, TASK_APP, RF_RX_WINDOW);
			ke_timer_set(APP_RF_INTV_TIMER, TASK_APP, RF_RX_INTV);
		}
#endif
...
KE_MSG_HANDLER_TAB(app)
{
    ...
    #if (HP_RF_CTRL_ENABLE)
    {APP_RF_TX_END_MSG,         (ke_msg_func_t)app_rf_tx_end_msg_handler},    
  
    {APP_RF_RX_END_MSG,         (ke_msg_func_t)app_rf_rx_end_msg_handler},    

    {APP_RF_RX_DATA_MSG,        (ke_msg_func_t)app_rf_rx_data_msg_handler},    

    {APP_RF_RX_WIND_TIMER,      (ke_msg_func_t)app_rf_rx_wind_handler},  

    {APP_RF_INTV_TIMER,         (ke_msg_func_t)app_rf_intv_handler},   
    #endif
    
    
    #if (BLE_APP_SEC)
    {APP_BOND_STORE_EVT,         (ke_msg_func_t)hp_sec_bond_store_evt_handler},
    #endif //(BLE_APP_SEC)
    #if (BLE_APP_HP_IUS)
