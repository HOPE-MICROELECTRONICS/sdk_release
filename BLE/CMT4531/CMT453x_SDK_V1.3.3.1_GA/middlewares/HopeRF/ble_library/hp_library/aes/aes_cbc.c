                   ...
                   uint8_t* out_message,  uint16_t message_len)
{
    ...
    env->iv          = iv;
    hp_ase_swap_array(env->key_cache,key,AES_BLOCK_SIZE); //swap array
    // Step 1: 
    ...
    {
        hp_ase_swap_array(env->val_cache,m_i,AES_BLOCK_SIZE);   //swap array
        // Start AES execution
        ...
        // Y := X XOR M_1
        // X := AES-128(K,Y)      
        hp_aes_xor_128_swap(env->val_cache, iv, m_i, AES_BLOCK_SIZE);
...
bool aes_cbc_continue(struct aes_cbc_env* env, uint8_t* aes_res)
{
    ...
    if(env->aes_env.src_info == DECRYPT_SRC_MARK)
    {
        hp_aes_xor_128_msb(o_i, env->iv, aes_res, AES_BLOCK_SIZE);
        env->iv = &(env->message[((env->cur_block) * AES_BLOCK_SIZE)]);
    }
    else{
        hp_ase_swap_array((void*)o_i, aes_res, AES_BLOCK_SIZE);
    }
    ...
    if(env->cur_block < env->num_blocks)
    {
        ...
        if(env->aes_env.src_info == DECRYPT_SRC_MARK)
        {
            ...
            // Y := X XOR M_i
            hp_aes_xor_128_swap(env->val_cache, o_i, m_i, AES_BLOCK_SIZE);
        }
        // X := AES-128(K,Y)
        hp_aes_start(&(env->aes_env), env->key_cache, env->val_cache);
    }
