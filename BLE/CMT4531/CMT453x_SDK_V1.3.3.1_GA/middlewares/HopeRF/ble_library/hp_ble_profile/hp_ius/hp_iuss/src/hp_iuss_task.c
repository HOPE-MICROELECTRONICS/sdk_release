 ...
 * @return 0 on success. 
 */
static int hp_ius_att_set_value(uint8_t att_idx, uint16_t length, const uint8_t *data)
{
    struct hp_ius_env_tag *hp_ius_env = PRF_ENV_GET(HP_IUS, hp_ius);
    struct hp_ius_val_elmt *val = (struct hp_ius_val_elmt *) co_list_pick(&(hp_ius_env->values));
    while (val != NULL)
    {
        if (val->att_idx == att_idx)
        {
            ...
            {
								co_list_extract(&hp_ius_env->values, &val->hdr);
                ke_free(val);
                val = NULL;
            }
            break;
        }
        val = (struct hp_ius_val_elmt *)val->hdr.next;
    }

    if (val == NULL)
    {
        val = (struct hp_ius_val_elmt *) ke_malloc(sizeof(struct hp_ius_val_elmt) + length, KE_MEM_ATT_DB);
        co_list_push_back(&hp_ius_env->values, &val->hdr);
    }
    val->att_idx = att_idx;
    val->length = length;
 ...
 */
static int hp_ius_att_get_value(uint8_t att_idx, uint16_t *length, const uint8_t **data)
{
    struct hp_ius_env_tag *hp_ius_env = PRF_ENV_GET(HP_IUS, hp_ius);
    struct hp_ius_val_elmt *val = (struct hp_ius_val_elmt *) co_list_pick(&hp_ius_env->values);
...
void hp_ius_init_ccc_values(const struct attm_desc_128 *att_db, int max_nb_att)
{
    ...
    for (int i = 1; i < max_nb_att; i++)
    {
			...
			{
				hp_ius_att_set_value(i, sizeof(ccc_values), ccc_values);
			}
	}
}
/** 
...
void hp_ius_set_ccc_value(uint8_t conidx, uint8_t att_idx, uint16_t ccc)
{
    ...
    hp_ius_att_get_value(att_idx, &length, &value);
    ASSERT_ERR(length);
    ASSERT_ERR(value);
    memcpy(new_value, value, length);
    new_value[conidx] = (uint8_t)ccc;
    hp_ius_att_set_value(att_idx, length, new_value);
}
/** 
 ...
 * @return Value of CCC. 
 */
static uint16_t hp_ius_get_ccc_value(uint8_t conidx, uint8_t att_idx)
{
    uint16_t length;
    const uint8_t *value;
    uint16_t ccc_value;
...
static uint8_t hp_ius_get_att_idx(uint16_t handle, uint8_t *att_idx)
{
    struct hp_ius_env_tag *hp_ius_env = PRF_ENV_GET(HP_IUS, hp_ius);
    uint8_t status = PRF_APP_ERROR;

    if ((handle >= hp_ius_env->shdl) && (handle < hp_ius_env->shdl + hp_ius_env->max_nb_att))
    {
        *att_idx = handle - hp_ius_env->shdl;
...
static int gattc_read_req_ind_handler(ke_msg_id_t const msgid, void const *param, ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
	...
	if(state == HP_IUS_IDLE)
	{
			...
			uint8_t status = hp_ius_get_att_idx(p_param->handle, &att_idx);
			uint16_t length = 0;
			uint16_t ccc_val = 0;

			if (status == GAP_ERR_NO_ERROR)
			{
					if( PERM_GET(hp_ius_att_db[att_idx].perm, UUID_LEN) == PERM_UUID_16 && (hp_ius_att_db[att_idx].uuid[0] + (hp_ius_att_db[att_idx].uuid[1]<<8) ) == ATT_DESC_CLIENT_CHAR_CFG)
					{
							ccc_val = hp_ius_get_ccc_value(conidx, att_idx);
							length = 2;
					}
					else
					{
							struct hp_ius_value_req_ind* req_ind = KE_MSG_ALLOC(HP_IUS_VALUE_REQ_IND,
																											prf_dst_task_get(&(hp_ius_env->prf_env), KE_IDX_GET(src_id)),
																											dest_id,
							...
							ke_msg_send(req_ind);
							ke_state_set(dest_id, HP_IUS_BUSY);
...
static int gattc_write_req_ind_handler(ke_msg_id_t const msgid, void const *param, ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
	...
	{
		if( PERM_GET(hp_ius_att_db[att_idx].perm, UUID_LEN) == PERM_UUID_16 &&
			(hp_ius_att_db[att_idx].uuid[0] + (hp_ius_att_db[att_idx].uuid[1]<<8) ) == ATT_DESC_CLIENT_CHAR_CFG)
		{			
				...
				if (status == ATT_ERR_NO_ERROR)
				{
						hp_ius_set_ccc_value(conidx, att_idx, *(uint16_t *)p_param->value);
				}
		}
		else
		{
		...
		{		
				...
				struct hp_ius_val_write_ind *req_id = KE_MSG_ALLOC_DYN(HP_IUS_VAL_WRITE_IND,
																								prf_dst_task_get(&(hp_ius_env->prf_env), KE_IDX_GET(src_id)),
																								dest_id, hp_ius_val_write_ind,
																								p_param->length);
...
static int gattc_att_info_req_ind_handler(ke_msg_id_t const msgid, void const *param, ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
	...
	{
		struct hp_ius_env_tag *hp_ius_env = PRF_ENV_GET(HP_IUS, hp_ius);
		struct hp_ius_att_info_req *req = KE_MSG_ALLOC(HP_IUS_ATT_INFO_REQ,
																									 TASK_APP,
																									 dest_id,
																									 hp_ius_att_info_req);
		req->conidx  = KE_IDX_GET(src_id);
		req->att_idx = p_param->handle - hp_ius_env->shdl;
		ke_msg_send(req);
	}	
	return KE_MSG_CONSUMED;
}	
...
void hp_ius_task_init(struct ke_task_desc *p_task_desc)
{
    // Get the address of the environment
    struct hp_ius_env_tag *p_hp_ius_env = PRF_ENV_GET(HP_IUS, hp_ius);

    p_task_desc->msg_handler_tab = hp_ius_msg_handler_tab;
    p_task_desc->msg_cnt         = ARRAY_LEN(hp_ius_msg_handler_tab);
    p_task_desc->state           = p_hp_ius_env->state;
    p_task_desc->idx_max         = HP_IUS_IDX_MAX;
}
